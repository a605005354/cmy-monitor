package cmy.monitor.cmyflink.utils;

/**
 * @BelongsProject: cmy-monitor
 * @BelongsPackage: cmy.monitor.cmyflink.utils
 * @Author: 蔡明勇
 * @CreateTime: 2022-09-09  14:18
...
public class RedisUtils {
    ...
    public boolean setex(String key, Object value, long expire) {
        try {//TimeUnit.SECONDS指定类型为秒
            log.error("redis set value and expire:{} {}", key, value);
     ...
     * @return
     */
    public Object getList(String key, int start, int end) {
        try {
            return redisTemplate.opsForList().range(key, start, end);
        } catch (Exception e) {
            log.error("redis lgetByIndex error,key:{},index:{}exception:{}", key, start, end, e);
            return null;
        }
    }
